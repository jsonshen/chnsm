...
public class SM4 {
	...
	private void setKey(long[] SK, byte[] key) {
		...
		k[3] = MK[3] ^ (long) FK[3];
		for (; i < 32; i++) {
			k[(i + 4)] = (k[i] ^ calcRk(k[(i + 1)] ^ k[(i + 2)] ^ k[(i + 3)] ^ (long) CK[i]));
			SK[i] = k[(i + 4)];
		}
	}
	...
	public byte[] ecbPadCrypt(SM4Context ctx, byte[] input) throws Exception {
		if (input == null) {
		...
		try (ByteArrayInputStream bins = new ByteArrayInputStream(input);
			...
			if (ctx.mode == DECRYPT) {
				ret = padding(ret, DECRYPT);
			}
			return ret;
		}
	}

	public byte[] cbcPadCrypt(SM4Context ctx, byte[] iv, byte[] input) throws Exception {
		...
		try (ByteArrayInputStream bins = new ByteArrayInputStream(input);
			...
			if (ctx.mode == ENCRYPT) {
				for (; len > 0; len -= 16) {
					byte[] in = new byte[16], out0 = new byte[16], out1 = new byte[16];
					bins.read(in);
					for (int i = 0; i < 16; i++) {
						out0[i] = ((byte) (in[i] ^ iv[i]));
					}
					oneRound(ctx.sk, out0, out1);
				...
				for (; len > 0; len -= 16) {
					byte[] in = new byte[16], out0 = new byte[16], out1 = new byte[16];
					bins.read(in);
					System.arraycopy(in, 0, tmp, 0, 16);
					oneRound(ctx.sk, in, out0);
					for (int i = 0; i < 16; i++) {
